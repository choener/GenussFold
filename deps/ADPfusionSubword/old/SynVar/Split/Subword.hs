...
instance
  ...
  mkStream grd (ls :!: Split _) (IStatic ()) hh (Subword (i:.j))
    = map (\s -> let RiSwI l = getIdx s
                 in  ElmSplitITbl Proxy () (RiSwI j) s (subword l j))
    $ mkStream grd ls (IVariable ()) hh (delay_inline Subword (i:.j)) -- TODO (see TODO in @Split@) - minSize c))
  mkStream grd (ls :!: Split _) (IVariable ()) hh (Subword (i:.j))
    = flatten mk step $ mkStream grd ls (IVariable ()) hh (delay_inline Subword (i:.j)) -- TODO (see above) - minSize c))
...
instance
  ...
  mkStream grd (ls :!: Split (TW (ITbl _ _ (_:.c) t) _)) (IStatic ()) hh (Subword (i:.j))
    = map (\s -> let RiSwI l = getIdx s
                     fmbkm :: mix = collectIx (Proxy :: Proxy uId) s :. subword l j
                 in  ElmSplitITbl Proxy (t ! fmbkm) (RiSwI j) s (subword l j))
    $ mkStream grd ls (IVariable ()) hh (delay_inline Subword (i:.j - minSize c))
  mkStream grd (ls :!: Split (TW (ITbl _ _ (_:.c) t) _)) (IVariable ()) hh (Subword (i:.j))
    = flatten mk step $ mkStream grd ls (IVariable ()) hh (delay_inline Subword (i:.j - minSize c))
...
instance
  ...
  mkStream grd (ls :!: Split (TW (BtITbl _ _) _)) (IStatic ()) hh (Subword (i:.j))
    = map (\s -> let RiSwI l = getIdx s
...
instance
  ...
  mkStream grd (ls :!: Split (TW (BtITbl (_:.c) t) bt)) (IStatic ()) hh (Subword (i:.j))
    ...
    $ mkStream grd ls (IVariable ()) hh (delay_inline Subword (i:.j - minSize c))
  mkStream grd (ls :!: Split (TW (BtITbl (_:.c) t) bt)) (IVariable ()) hh (Subword (i:.j))
    = flatten mk step $ mkStream grd ls (IVariable ()) hh (delay_inline Subword (i:.j))
    where mk s = let RiSwI l = getIdx s in return (s :. (delay_inline id $ j - l - minSize c))
          step (s:.z) | z >= 0 = do let RiSwI k      = getIdx s
                                        l            = j - z
                                        kl           = subword k l
